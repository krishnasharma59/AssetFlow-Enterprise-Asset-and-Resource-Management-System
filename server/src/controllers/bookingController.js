...
const createBooking = asyncHandler(async (req, res) => {
  ...
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return res.status(400).json({ message: 'Enter valid booking start and end times' });
  if (end <= start) return res.status(400).json({ message: 'Booking end time must be after its start time' });
