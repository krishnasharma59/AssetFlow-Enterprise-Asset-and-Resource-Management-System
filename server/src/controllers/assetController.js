...
// Assets with operational history must be retained for ERP traceability.
const deleteAsset = asyncHandler(async (req, res) => {
  const asset = await Asset.findById(req.params.id);
  if (!asset) return res.status(404).json({ message: 'Asset not found' });

  const [allocations, bookings, maintenance, auditItems] = await Promise.all([
    Allocation.countDocuments({ asset: asset._id }),
    Booking.countDocuments({ resource: asset._id }),
    MaintenanceRequest.countDocuments({ asset: asset._id }),
    AuditItem.countDocuments({ asset: asset._id }),
  ]);

  if (allocations || bookings || maintenance || auditItems) {
    return res.status(409).json({
      message: 'This asset has operational history and cannot be deleted. Mark it Retired or Disposed instead to preserve the audit trail.',
    });
  }

  await asset.deleteOne();
  await logActivity(req.user._id, 'Deleted asset', 'Asset', asset._id, `${asset.assetTag} - ${asset.name}`);
